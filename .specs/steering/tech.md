...
| D22 | Dashboard session = **backend-minted JWT** signed with the session key, presented as an `Authorization: Bearer` token — **not a cookie** (SWA and Function App are different origins; Functions validate the JWT statelessly per request). Supersedes "session cookie" wording. *(Amended 2026-07-16: the cross-origin Bearer fetch requires CORS at two layers — the Functions host answers `OPTIONS` preflights from the platform CORS allowlist and never forwards them to the worker, so app-level (FastAPI) CORS alone fails silently in prod. Platform allowlist is template-managed in the infra repo, in lockstep with `CORS_ALLOWED_ORIGIN` — deployment Bugfix log #6.)* | **Decided** |
