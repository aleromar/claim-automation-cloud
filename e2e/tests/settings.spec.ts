...
test("save Trello settings, persist across reload, secrets write-only", async ({ page }) => {
  ...
  await page.getByLabel(/id del tablero/i).fill(boardId);
  await page.getByLabel(/id de la lista/i).fill(listId);
  await page.getByLabel(/clave api/i).fill("e2e-trello-key");
  await page.getByRole("button", { name: /guardar ajustes de trello/i }).click();

  // Response re-render: badge flips to stored, input returns to empty (write-only).
  await expect(page.getByLabel(/clave api \(guardado\)/i)).toHaveValue("");
  ...
  await page.reload();
  await expect(page.getByLabel(/id del tablero/i)).toHaveValue(boardId);
  await expect(page.getByLabel(/id de la lista/i)).toHaveValue(listId);
  await expect(page.getByLabel(/clave api \(guardado\)/i)).toHaveValue("");
});
...
test("reconnect Gmail re-consents and replaces the stored refresh token (REQ-3)", async ({
  ...
  await page.getByRole("link", { name: "Ajustes" }).click();
  await expect(page.getByText(/token de actualización: guardado/i)).toBeVisible();
  // Top-level navigation through the Vite proxy → backend 302 → stub consent.
  await page.getByRole("button", { name: /reconectar gmail/i }).click();
